<!DOCTYPE html>
<html lang="en">
@include('dashboard.donor.layout.head')
<style>
    .badge{
...
<body>
@include('dashboard.donor.layout.header')
...
@if(Auth::guard('donor')->check())
    <aside class="sidebar">
        <ul>
            ...
            <li>
                <a href="{{ route('donor.showNotifications') }}"
                   class="{{ request()->routeIs('donor.showNotifications') ? 'active' : '' }}">
                  Notifications <i class="fa-regular fa-bell"></i>
                  @php $unread = auth('donor')->user()->unreadNotifications->count(); @endphp
                  @if($unread > 0)
                    <span class="badge">{{ $unread }}</span>
                  @endif
                </a>
              </li>
              <li><a href="{{route('donor.setting')}}" class="{{ request()->routeIs('donor.setting') ? 'active' : '' }}">Settings <i class="fa-solid fa-gear"></i></a></li>
        </ul>
    </aside>
@endif
