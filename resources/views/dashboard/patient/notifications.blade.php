...
<div class="dashboard-container" style="width: 53pc;">
    ...
    @auth('patient')
    @forelse (auth('patient')->user()->notifications as $notification)
    ...
    <div class="notification" style="margin-top: 15px">
      ...
      <div class="notification-timestamp">{{ $notification->data['date'] }} 
        ...
        <a href="javascript:void(0)" onclick="document.getElementById('deleteNotify{{$notification->id}}').submit()" class="action-link delete"><i class="fa-regular fa-trash-can"></i></a>
        <form id="deleteNotify{{$notification->id}}" action="{{route('patient.deleteNotification', $notification->id)}}" method="POST">
          @csrf
          @method('DELETE')
        </form>
